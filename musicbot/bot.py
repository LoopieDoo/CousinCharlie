...
class MusicBot(discord.Client):
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                ...
                newmsg = 'Mirdza sucks big juicy dick in %s: **%s**' % (
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            if permissions.max_song_length and info.get('duration', 0) > permissions.max_song_length:
                raise exceptions.PermissionsError(
                    "Mircea prostu (%s > %s)" % (info['duration'], permissions.max_song_length),
                    expire_in=30
                )
    ...
    async def cmd_halloffame(self):
            quotes = ["*xd* - SoftMandar 2013", "*5 lei* - Softmandar 2013", "*zboara in gard si mor* - Loopie 2016", "*ba astia fac baronu* - Nihtmer 2016", "*sterge-ma*",
                      "*spargem telefone 2* - Nihtmer 2013",
                      "*Hai afara Mircea fututi capu tau sa-ti fut tot mama si toti cainii si pe Anca si pe toti si altaru ma-tii si baga-mi-as pula in gura la Anca si pe taicatu si pe toata rasa ta de* - SoftMandar 2013.", "*Astazi vom face un gameplay de LoL 1v1 putin mai troll cu Soraka* - Denis 2012"]
            random.shuffle(quotes)
            for i in quotes:
                return Response(i)

    async def cmd_wtf(self):
            array = ["aaa fuck my ass with steak", "dusky is gay", "buna eu sunt lepa si imi place sa sug pula", "Charlie,play https://www.youtube.com/watch?v=DMpYQCd9z3Q", "Charlie,play https://www.youtube.com/watch?v=eV-hZZmN-1o "]
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        if player.is_stopped:
            raise exceptions.CommandError("daca citesti asta esti gay", expire_in=20)
    ...
    async def cmd_setavatar(self, message, url=None):
        ...
        return Response("PISH", delete_after=20)
